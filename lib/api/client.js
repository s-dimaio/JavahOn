...
class HonAPI {
  ...
  async loadCommands(appliance) {
    ...
    if (!appliance.applianceType && !appliance.info.applianceTypeName) {
      console.log('⚠️  Appliance type missing, skipping commands load');
      return {};
    }
    ...
    console.log('🔍 Loading commands with params:', JSON.stringify(params, null, 2));
    ...
    try {
      ...
      if (!result || result.resultCode !== '0') {
        console.log('⚠️  Commands load returned non-zero result code or empty response');
        console.log('   This might be a virtual/unsupported device');
        return {};
      }
      
      delete result.resultCode;
      console.log(`✅ Commands loaded successfully (${Object.keys(result).length} items)`);
      return result;
    } catch (error) {
      console.error('⚠️  Load commands error:', error.response?.data?.error || error.message);
      console.log('   Returning empty commands (device might not support commands)');
