...
class HonAPI {
   ...
   *   - state.machMode.2: "Running" (machine states)
   *   - state.prPhase.11: "Spin" (program phases)
   *   - parameters.temp: "Temperature" (parameter names)
   * @example
   * const translations = await api.getTranslations('it');
   * console.log(translations['programs.wm.cottons']); // "Cotone"
   * console.log(translations['state.machMode.2']); // "In funzione"
   */
  async getTranslations(language = 'en') {
    try {
      const url = `${constants.API_URL}/translation`;
      const params = {
        language: language,
        beta: true
      };
      ...
      // API returns translations directly in payload
      if (response.data?.payload) {
        this._debugLog(`✅ Loaded ${Object.keys(response.data.payload).length} translations for language: ${language}`);
        return response.data.payload;
      }
      
      this._debugLog('⚠️  No translations found in response');
      return {};
    } catch (error) {
