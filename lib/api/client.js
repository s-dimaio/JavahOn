...
class HonAPI {
   ...
   * Emits detailed debug logs about the raw API response to help diagnose
   * cases where the server returns 0 appliances or an unexpected structure.
   * @public
   * @returns {Promise<Array>} List of appliances
   * @throws {HonAuthenticationError} If the HTTP request fails
   * @example
  ...
  async loadAppliances() {
    try {
      const deviceId = this._auth && this._auth.device ? this._auth.device.mobileId : 'JavahOn';
      const response = await this._hon.post(
        `${constants.API_URL}/unified-api/v1/view/appliance-list`,
        { deviceId }
      );
      const result = response.data;

      this._debugLog(`📡 loadAppliances() - HTTP status: ${response.status}`);

      if (result && result.success && result.modules && result.modules.applianceList && result.modules.applianceList.success) {
        const payload = result.modules.applianceList.payload;
        if (payload && payload.appliances) {
          const list = payload.appliances;
          this._debugLog(`📡 loadAppliances() - Found ${list.length} appliance(s)`);
          list.forEach((a, i) => {
            this._debugLog(`📡   [${i}] type=${a.applianceTypeName || a.applianceType || '?'} | mac=${a.macAddress || a.serialNumber || '?'} | name=${a.nickName || a.modelName || '?'} | brand=${a.brand || '?'}`);
          });
          return list;
        }
      } else {
        this._debugLog(`⚠️  loadAppliances() - Unexpected response structure. Full response data: ${JSON.stringify(result).substring(0, 500)}`);
      }

      this._debugLog('⚠️  loadAppliances() - Returning empty array (no appliances found in response)');
      return [];
    } catch (error) {
      if (error.response) {
        this._debugLog(`❌ loadAppliances() - HTTP ${error.response.status} error`);
        this._debugLog(`❌ loadAppliances() - Response headers: ${JSON.stringify(error.response.headers)}`);
        this._debugLog(`❌ loadAppliances() - Response data: ${JSON.stringify(error.response.data).substring(0, 500)}`);
      }
  ...
  async loadCommands(appliance) {
    ...
    try {
      ...
      this._debugLog('⚠️  Load commands error:', error.response?.data?.error || error.message);
  ...
  async loadLastActivity(appliance) {
    try {
      ...
      this._debugLog('Failed to load last activity:', error.message);
      return {};
    }
  }
  ...
  async loadMaintenance(appliance) {
    try {
      ...
      this._debugLog('Failed to load maintenance:', error.message);
      return {};
    }
  }
