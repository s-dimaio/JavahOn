  ...
  _setupCookieInterceptors() {
    ...
    this._session.interceptors.request.use(async (config) => {
      if (config.url) {
        ...
        if (cookieString) {
          console.log(`🍪 Sending cookies to ${config.url.substring(0, 60)}...`);
          console.log(`🍪 Cookie count: ${cookieString.split(';').length} cookies`);
          config.headers.Cookie = cookieString;
        } else {
          console.log(`⚠️ No cookies to send to ${config.url.substring(0, 60)}...`);
        }
      }
      return config;
    });
    ...
    this._session.interceptors.response.use(
      async (response) => {
        ...
        if (setCookieHeaders && response.config.url) {
          console.log(`🍪 Received ${setCookieHeaders.length} cookies from ${response.config.url.substring(0, 60)}...`);
          for (const cookie of setCookieHeaders) {
            await this._cookieJar.setCookie(cookie, response.config.url);
            console.log(`🍪 Stored: ${cookie.split(';')[0]}`);
          }
        }
        return response;
      },
