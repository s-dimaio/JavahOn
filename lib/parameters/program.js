...
class HonParameterProgram extends HonParameterEnum {
   ...
   * @param {Object} command - Command object (HonCommand instance)
   * @param {string} group - Parameter group
   */
  constructor(key, command, group) {
    // Pass empty attributes to super as we handle data differently
    super(key, {}, group);
    this._command = command;
    this._programs = command.categories || {};
    this._value = '';
    this._values = [];
    this._typology = 'enum';
    this._setAttributes();
  ...
  _setAttributes() {
    ...
    // Program parameter value is derived from command category
    if (this._command && this._command.category) {
      if (this._command.category.includes('PROGRAM')) {
        this._value = this._command.category.split('.').pop().toLowerCase();
      } else {
        this._value = this._command.category;
      }
    }

    if (!this._value && this.values.length > 0) {
  ...
  get values() {
    ...
    if (this._programs && typeof this._programs === 'object') {
      const filter = ['iot_recipe', 'iot_guided'];
      currentValues = Object.keys(this._programs).filter(key =>
        filter.every(f => !key.includes(f))
      ).sort();

      // Debug logging - moved here if needed, but not in the instruction
      if (currentValues.length === 0) {
        console.log('[HonParameterProgram] Warning: No values found!');
        console.log('[HonParameterProgram] Keys found:', Object.keys(this._programs));
        console.log('[HonParameterProgram] Categories object:', typeof this._programs);
        console.log('[HonParameterProgram] Is Command:', this._command ? this._command.name : 'null');
      }
    }
