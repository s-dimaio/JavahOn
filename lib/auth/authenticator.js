...
class HonAuth extends EventEmitter {
   ...
   * @param {boolean} [debug=false] - Enable debug logging (console.log output)
   * @example
   ...
   * const auth = new HonAuth(null, 'user@email.com', 'myPassword', null, true);
   */
  constructor(session, email, password, device, debug = false) {
    super();
    this._debug = debug;
    ...
    this._request = new HonAuthConnectionHandler(this._session, debug);
   ...
   * @public
   * @returns {HonDevice} Device information
   */
  get device() {
    return this._device;
  }
  ...
  async _introduce() {
    ...
    this._debugLog('🔗 OAuth URL:', url.substring(0, 100) + '...');
    ...
    try {
      ...
      this._debugLog('✅ Login URL extracted:', loginUrl.substring(0, 100) + '...');
      return loginUrl;
    } catch (error) {
  ...
  async _login() {
    ...
    this._debugLog('📮 Sending login POST to:', `${constants.AUTH_API}/s/sfsites/aura?${paramsString}`);
    this._debugLog('📦 Form data length:', formDataString.length);
    ...
    try {
      ...
      this._debugLog('📤 Login response status:', response.status);
      this._debugLog('📦 Login response data type:', typeof response.data);
      this._debugLog('📦 Login response preview:', JSON.stringify(response.data).substring(0, 200));

      if (response.status === 200) {
        try {
          const result = response.data;
          this._debugLog('🔍 Result structure:', Object.keys(result));
          ...
          if (result.events && result.events[0] && result.events[0].attributes && result.events[0].attributes.values) {
            ...
            this._debugLog('✅ Login successful, extracted URL:', url.substring(0, 100) + '...');
            return url;
          } else {
  ...
  async _getToken(url) {
    try {
      ...
      if (this._request._session._cookieJar) {
        ...
        this._debugLog(`🔍 Cookies in jar before _getToken GET: ${cookies.length} cookies`);
        cookies.forEach(c => this._debugLog(`   - ${c.key}=${c.value.substring(0, 30)}...`));
      }
      ...
      this._debugLog('🔗 Getting tokens from:', finalUrl.substring(0, 100) + '...');
  ...
  setTokens(tokens) {
    ...
    this._debugLog(`setTokens: Tokens loaded successfully (${timeRemaining} minutes until expiration)`);
