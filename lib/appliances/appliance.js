...
class HonAppliance {
  ...
  constructor(api, info, zone = 0) {
    ...
    // Load appliance-specific extras
    try {
      const applianceType = (info.applianceTypeName || info.applianceType || '').toLowerCase();
      const ApplianceClass = require(`./types/${applianceType}`);
      this._extra = new ApplianceClass(this);
    } catch (error) {
      // No specific appliance class found, use defaults
      this._extra = null;
    }
  }
  ...
  _debugLog(...args) {
    try {
      if (this._api && this._api._honHandler && this._api._honHandler._auth && this._api._honHandler._auth._debug) {
        console.log(...args);
      }
    } catch (error) {
  ...
  /**
   * Get appliance-specific extra class instance (e.g., WashingMachine)
   * @returns {Object|null} Extra class instance
   */
  get extra() {
    return this._extra;
  }
  ...
  async loadAttributes() {
    ...
    // Update connection status from lastConnEvent
    if (this._attributes.lastConnEvent) {
      this._connection = this._attributes.lastConnEvent.category !== 'DISCONNECTED';
    }
  ...
  /**
   * Update appliance data from MQTT payload
   * Handles real-time parameter updates from MQTT broker
   ...
   * appliance.updateFromMQTT({
   *   parameters: { machMode: '2', prPhase: '4', remainingTimeMM: 45 }
   * });
   */
  updateFromMQTT(payload) {
    if (!payload || !payload.parameters) {
      this._debugLog('⚠️ updateFromMQTT: Invalid payload received');
      return;
    }

    const params = payload.parameters;
    this._debugLog(`📥 updateFromMQTT: Received ${Object.keys(params).length} parameters`);

    // Capture old values BEFORE updating attributes (for state transition detection)
    const oldValues = {};
    if (this._attributes && this._attributes.parameters) {
      for (const key of Object.keys(params)) {
        if (this._attributes.parameters[key]) {
          oldValues[key] = this._attributes.parameters[key].value;
        }
      }
    }

    // Update internal attributes with MQTT data
    if (this._attributes && this._attributes.parameters) {
      for (const [key, value] of Object.entries(params)) {
        if (this._attributes.parameters[key]) {
          this._debugLog(`   ✏️  Updating ${key}: ${this._attributes.parameters[key].value} → ${value}`);
          this._attributes.parameters[key].value = value;
        } else {
          // Create dynamic parameter (e.g., delayTime only exists during runtime)
          this._debugLog(`   ➕ Creating dynamic parameter ${key} = ${value}`);
          this._attributes.parameters[key] = { value };
        }
      }
    }

    // Attach old values to payload for subclasses (e.g., WashingMachine state detection)
    payload.oldValues = oldValues;

    // If we receive an MQTT message, the appliance is online
    const wasOffline = !this._connection;
    this._connection = true;

    if (wasOffline) {
      this._debugLog('📡 Appliance came online (MQTT message received)');
    }

    // Note: Subclasses (e.g., WashingMachine) should override this method
    // to call updateState() for event emission after calling super.updateFromMQTT()
  }
  ...
  syncParamsToCommand(commandName) {
    ...
    if (!command) {
      this._debugLog(`🔍 syncParamsToCommand: Command '${commandName}' not found`);
      return;
    }

    this._debugLog(`🔍 syncParamsToCommand: Syncing command '${commandName}' with ${command.settingKeys.length} setting keys`);

    for (const key of command.settingKeys) {
      const newParam = this.attributes.parameters?.[key];

      this._debugLog(`🔍 Processing key '${key}': newParam=${newParam ? 'exists' : 'missing'}, value='${newParam?.value || 'N/A'}'`);

      if (!newParam || newParam.value === '') {
        this._debugLog(`  ⏭️ Skipping ${key} (no param or empty value)`);
        continue;
      }
      ...
      if (!setting) {
        this._debugLog(`  ⚠️ Setting ${key} not found in command ${commandName}`);
        continue;
      }

      this._debugLog(`  ✅ Setting found for ${key}, type: ${setting.constructor.name}`);

      try {
        if (!(setting instanceof HonParameterRange)) {
          this._debugLog(`  📝 Setting ${key} = "${String(newParam.value)}" (string)`);
          setting.value = String(newParam.value);
        } else {
          this._debugLog(`  📝 Setting ${key} = ${parseFloat(newParam.value)} (number)`);
          setting.value = parseFloat(newParam.value);
        }
        this._debugLog(`  ✅ Successfully set ${key}`);
      } catch (error) {
        this._debugLog(`  ❌ Can't set ${key} - ${error.message}`);
      }
    }

    this._debugLog(`✅ syncParamsToCommand completed for '${commandName}'`);
  }
