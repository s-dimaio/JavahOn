...
class WashingMachine extends ApplianceBase {
  ...
  updateState(params) {
    ...
    // Lazy initialization: sync state from current attributes on first call
    // This ensures state transitions are detected correctly from the very first MQTT update
    // without requiring explicit syncState() call by library consumers
    if (this._previousMachMode === null || this._previousPrPhase === null) {
      ...
      console.log(`[WM] Auto-sync state on first update: machMode=${this._previousMachMode}, prPhase=${this._previousPrPhase}`);
    }
