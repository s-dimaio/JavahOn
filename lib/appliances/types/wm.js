...
class WashingMachine extends ApplianceBase {
  ...
  /**
   * Synchronize internal state tracking with current appliance attributes
   * Must be called after loading attributes from API to ensure state transitions
   * are correctly detected from the first MQTT update onwards.
   * 
   * Without calling this, _previousMachMode starts as null and the first
   * state transition (e.g., running → finished) will be missed because
   * null === 2 evaluates to false.
   * 
   * @returns {void}
   * @example
   * // After loading attributes from API
   * await appliance.loadAttributes();
   * appliance.extra.syncState();
   * // Now programStarted/programFinished will fire correctly
   */
  syncState() {
    const params = this.parent.attributes?.parameters || {};
    
    const machModeParam = params.machMode;
    if (machModeParam !== undefined) {
      const val = typeof machModeParam === 'object' ? machModeParam.value : machModeParam;
      this._previousMachMode = val !== null && val !== undefined ? parseInt(val) : null;
    }

    const prPhaseParam = params.prPhase;
    if (prPhaseParam !== undefined) {
      const val = typeof prPhaseParam === 'object' ? prPhaseParam.value : prPhaseParam;
      this._previousPrPhase = val !== null && val !== undefined ? parseInt(val) : null;
    }

    console.log(`[WM] State synced from attributes: machMode=${this._previousMachMode}, prPhase=${this._previousPrPhase}`);
  }
