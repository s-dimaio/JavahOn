...
class WashingMachine extends ApplianceBase {
  ...
  getLocalizedProgramName(programId, translationKey = null) {
    ...
    // Try translation using key provided by API
    if (translationKey) {
   ...
   * @returns {Object|null} Program object with id, name, prCode, prPosition, translationKey, isIot, isSpecial or null if not found
   */
  findProgramByCode(prCode, prPosition = null, remoteControlEnabled = null) {
    ...
    for (const [key, category] of Object.entries(startCmd.categories)) {
      ...
      const translationKey = category.parameters?.program?.category;
