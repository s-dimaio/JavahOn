...
class WashingMachine extends ApplianceBase {
   ...
   * Excludes programs without prCode and favourites
  ...
  getAvailablePrograms() {
    ...
    for (const [name, category] of Object.entries(startCmd.categories)) {
      ...
      // Get localized name using internal method
      const displayName = this.getLocalizedProgramName(name);
  ...
  getProgramTranslationKey(programId) {
    ...
    return `PROGRAMS.WM_WD.${normalizedId}`;
  }

  /**
   * Get localized program name
   * Uses internal translations, falls back to formatted name if not available
   * @param {string} programId - Program ID from API
   * @returns {string} Localized program name or formatted fallback
   * @example
   * const name = wm.getLocalizedProgramName('rapid_14_min');
   * // Returns: "Rapido 14'" (if translated) or "Rapid 14 Min" (fallback)
   */
  getLocalizedProgramName(programId) {
    if (!programId) return '';
