...
class WashingMachine extends EventEmitter {
  ...
  findProgramByCode(prCode, prPosition = null) {
    ...
    for (const [key, category] of Object.entries(startCmd.categories)) {
      ...
      // Match by prCode first
      if (catPrCode !== searchPrCode) continue;
      
      // If searchPrPosition is provided, must match exactly
      if (searchPrPosition !== null) {
        if (catPrPosition === searchPrPosition) {
          return {
            id: key,
        ...
        // No prPosition provided, return first match with prCode
        return {
          id: key,
          name: HonParameterProgram.formatProgramName(key),
          prCode: parseInt(prCode),
