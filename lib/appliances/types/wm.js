 ...
 * Extends EventEmitter to provide real-time state change events
 */

const ApplianceBase = require('./base');
const EventEmitter = require('events');

class WashingMachine extends EventEmitter {
  ...
  updateState(params) {
    ...
    const machMode = params.machMode?.value !== undefined ? parseInt(params.machMode.value) : null;
    const prPhase = params.prPhase?.value !== undefined ? parseInt(params.prPhase.value) : null;
    const program = params.prStr?.value || params.prCode?.value || null;
