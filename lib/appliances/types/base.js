    ...
    try {
      ...
      if (prCode) {
        ...
        if (this.parent._extra && typeof this.parent._extra.findProgramByCode === 'function') {
          // Extract temp and spinSpeed for custom favourite disambiguation
          const tempParam = data.parameters?.temp;
          const spinParam = data.parameters?.spinSpeed;
          const temp = tempParam ? parseInt(String(tempParam.value ?? tempParam)) : null;
          const spinSpeed = spinParam ? parseInt(String(spinParam.value ?? spinParam)) : null;
          const program = this.parent._extra.findProgramByCode(prCode, prPosition, null, temp, spinSpeed);
