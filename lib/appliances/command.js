...
class HonCommand {
  ...
  async sendParameters(params) {
    ...
    // Always set prStr with the localized program name
    // This is important for the hOn API to display the correct program name
    console.log(`🔍 [JavahOn] Setting prStr for program`);
    console.log(`🔍 [JavahOn] _categoryName: "${this._categoryName}"`);

    // Try to get localized program name from appliance extra (e.g., WashingMachine)
    if (this._appliance._extra && typeof this._appliance._extra.getLocalizedProgramName === 'function') {
      const programId = this._categoryName.split('.').pop().toLowerCase().replace(/_/g, '_');
      console.log(`🔍 [JavahOn] Calling getLocalizedProgramName with programId: "${programId}", translationKey: "${this._categoryName}"`);
      ...
      console.log(`🔍 [JavahOn] Localized name returned: "${localizedName}"`);
      params.prStr = localizedName || this._categoryName.toUpperCase();
    } else {
      console.log(`🔍 [JavahOn] No extra or getLocalizedProgramName function, using categoryName`);
      // Fallback to category name if no translation available
