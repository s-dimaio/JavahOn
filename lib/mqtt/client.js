...
class MQTTClient extends EventEmitter {
    ...
    async connect() {
        try {
            ...
            console.log(`   tokenSigned: ${tokenSigned ? tokenSigned.substring(0, 50) + '...' : 'MISSING'}`);
            ...
            console.log(`   endpoint: ${constants.AWS_ENDPOINT}`);
    ...
    _handleApplianceStatusUpdate(topic, payload) {
        ...
        if (payload.parameters) {
            for (const [key, value] of Object.entries(payload.parameters)) {
                if (appliance.attributes.parameters[key] !== undefined) {
                    const oldValue = appliance.attributes.parameters[key];
                    appliance.attributes.parameters[key] = value;
                    
                    // Update corresponding command parameters
                    appliance.syncCommand(key, value);
                    
                    console.log(`[${macAddress}] ${key}: ${oldValue} → ${value}`);
                }
            }
        }
    ...
    _handleConnectionEvent(topic, payload, connected) {
        ...
        if (appliance) {
            ...
            console.log(`[${macAddress}] ${connected ? 'Connected' : 'Disconnected'}`);
    ...
    _handleDiscoveryUpdate(topic, payload) {
        console.log('Discovery update:', topic, payload);
    ...
    _startWatchdog() {
        ...
        this._watchdogInterval = setInterval(async () => {
            if (!this._connected && !this._reconnecting) {
                console.log('Watchdog: Connection lost, attempting to reconnect...');
