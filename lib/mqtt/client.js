...
class MQTTClient extends EventEmitter {
    ...
    _handleApplianceStatusUpdate(topic, payload) {
        ...
        if (payload.parameters) {
            // Check if parameters is an array (new MQTT format) or object (old format)
            let params;
            ...
            console.log(`   Processing ${Object.keys(params).length} parameters...`);
            
            // Ensure attributes.parameters exists
            if (!appliance.attributes) {
                console.log(`   ⚠️  Creating attributes object for appliance`);
                appliance.attributes = { parameters: {} };
            }
            if (!appliance.attributes.parameters) {
                console.log(`   ⚠️  Creating parameters object for appliance`);
                appliance.attributes.parameters = {};
            }
            ...
            // Update payload.parameters to object format for event emission
