...
class MQTTClient extends EventEmitter {
    ...
    _handleApplianceStatusUpdate(topic, payload) {
        ...
        // Convert array-based parameters to object format if needed
        if (payload.parameters) {
            if (Array.isArray(payload.parameters)) {
                console.log(`   Converting array-based parameters to object...`);
                // Convert array format to object: [{parName: "x", parNewVal: "y"}] => {x: y}
                const params = {};
                for (const param of payload.parameters) {
                    if (param.parName && param.parNewVal !== undefined) {
                        params[param.parName] = param.parNewVal;
                    }
                }
                payload.parameters = params;
                console.log(`   Converted parameters:`, JSON.stringify(params, null, 2));
            }

            console.log(`   Processing ${Object.keys(payload.parameters).length} parameters...`);

            // Let the appliance handle its own state updates
            // This will update attributes AND trigger events (programStarted, etc.)
            // Note: For specific appliance types (WashingMachine, etc.), the instance is in appliance.extra
            const target = appliance.extra || appliance;

            if (typeof target.updateFromMQTT === 'function') {
                console.log('   📤 Calling updateFromMQTT()...');
                target.updateFromMQTT(payload);
                console.log('   ✅ Appliance updated via updateFromMQTT');
            } else {
                console.warn('   ⚠️  updateFromMQTT not available, skipping update');
            }
        }

        console.log('   📤 Emitting applianceUpdate event...');
        // Still emit event for backward compatibility (e.g., connection status monitoring)
